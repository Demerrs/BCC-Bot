...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    if (isNaN(msDuration)) {
      await interaction.editReply('Enter a valid mute duration.');
      return;
    }
    ...
    try {
      ...
      if (targetUser.isCommunicationDisabled()) {
        ...
        await interaction.editReply(`${targetUser}'s mute has been updated to ${prettyMs(msDuration, { verbose: true })}\nReason: ${reason}`);
        return;
      }

      await targetUser.timeout(msDuration, reason);
      await interaction.editReply(`${targetUser} was muted out for ${prettyMs(msDuration, { verbose: true })}.\nReason: ${reason}`);
    } catch (error) {
      console.log(`There was an error when muting out: ${error}`);
    }
  },
